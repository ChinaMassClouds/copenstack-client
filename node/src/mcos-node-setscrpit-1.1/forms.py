...
def needCheckLicense():
    probation_file = '/usr/local/probation'
    license_file = '/usr/local/license_date'
    return os.path.exists(probation_file) or os.path.exists(license_file)

def inLicenseDate():
    try:
        probation_file = '/usr/local/license_date'
...
class Login(django_auth_forms.AuthenticationForm):
    ...
    @sensitive_variables()
    def clean(self):
        if needCheckLicense() and not inProbation() and not inLicenseDate():
            self.request.session.flush()
            raise forms.ValidationError(_('No license or license-date is out.'))
